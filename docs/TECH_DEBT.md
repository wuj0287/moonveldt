# Moonveldt 技术债与未来路线备忘（TECH_DEBT）

> 建立于 2026-09-05（v1.9.1 封版时点）。本文件的目的：**把第三方评审（GPT）中被采纳的技术认知固化下来**，防止重启项目时从头重新发现。Moonveldt 当前处于冻结状态——记录在案，但**冻结期内不偿还任何一项**。
...
现状：每个代码块独立**进程**，但共享同一个 venv（`%APPDATA%\Moonveldt\pyenv`）。

- 这对 Notebook 式体验是**刻意选择**：Block A 装完 torch，Block B 直接 import 可用
- 若未来做"项目级开发"，需要升级为：全局 Runtime → 项目 Environment → 文档 Environment → 代码块 Process，env 落到 `项目/.Moonveldt/pyenv`
...
### 5. .Moonveldt/ 元数据目录（未来项目的数据规范）

坚持 .md 永远是用户真正拥有的数据（无 vendor lock-in 是长期优势）；运行结果、env、cache 等元数据应放项目下 `.Moonveldt/` 目录，不污染文档本体。

## 二、未来功能 backlog（来自 GPT 评审，已裁决）

重启 Moonveldt 前这些一律不做。工程量为当时评估值。
